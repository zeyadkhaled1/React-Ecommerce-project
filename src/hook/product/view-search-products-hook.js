...
const ViewSearchProductsHook = () => {
	...
	const getProductSearched= async () =>{
		...
		sortData()
		dispatch(getProducts(`?name=${word}&from=${priceFrom}&to=${priceTo}&brand=${queryBrand}&category=${searchCategoryid}&sort=${sort}&pageSize=12`))
	}
	const onPress = async page => {
		...
		let priceTo=localStorage.getItem("priceTo")
		priceTo = isNaN(parseInt(priceTo)) ? "" :Math.max(0, parseInt(priceTo));
		let searchCategoryid= localStorage.getItem("Categoryid").join(",") || ""
		console.log(searchCategoryid)
		sortData()
		await dispatch(getProducts(`?name=${word}&from=${priceFrom}&to=${priceTo}&brand=${queryBrand}&category=${searchCategoryid}&sort=${sort}&pageNumber=${page}&pageSize=12`));
	};
	let sort;
	const sortData=()=>{
