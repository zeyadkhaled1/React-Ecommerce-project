...
export const SideFilter = () => {
  ...
  const handleCategoryClick = (category,SubCatArr) => {
    ...
    if (activeCategory !== category){
      if (SubCatArr !=='0'){
        const idArray = SubCatArr.map(item => item._id);
        searchCat(idArray);
      }
      else{
        searchCat(SubCatArr);
    ...
    <div className="mt-3">
      <Row>
        <div className="d-flex flex-column mt-2">
          ...
          <div className='list-basic mt-3' onClick={() => handleCategoryClick('0','0')} >الكل&gt;&gt;</div>
          {categoryToShow ? (
            <AnimatePresence>
                  ...
                  >
                    <ul className="mx-0 my-0 px-0">
                      <li
                        ...
                        onClick={() => handleCategoryClick(item.title,SubCat[item.title])}
                      >
                        {item.title} &gt;
                      </li>
                      {SubCat[item.title] && SubCat[item.title].length > 0 && activeCategory === item.title ? (
                        <ul>
                          <AnimatePresence>
                            {SubCat[item.title].map((item2, index2) => (
                              <motion.li
                                key={index2}
                                onClick={()=>{handleSubCategoryClick(item2._id)}}
